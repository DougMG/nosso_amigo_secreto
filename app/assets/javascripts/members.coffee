  ...
  $('body').on 'click', 'a.remove_member', (e) ->
    $.ajax '/members/'+ e.currentTarget.id,
        ...
        success: (data, text, jqXHR) ->
          M.toast({html:'Membro removido', classes: 'rounded tael lighten-2', 4000})
          $('#member_' + e.currentTarget.id).remove()
        error: (jqXHR, textStatus, errorThrown) ->
          M.toast({html:'Problema na remoção de membro', classes: 'rounded red accent-2', 4000})
    return false
  ...
  $('.new_member').on 'submit', (e) ->
    $.ajax e.target.action,
        ...
        success: (data, text, jqXHR) ->
          ...
          $('#member_name').focus()
          M.toast({html:'Membro adicionado', classes: 'rounded tael lighten-2', 4000})
        error: (jqXHR, textStatus, errorThrown) ->
          M.toast({html:'Problema na hora de incluir membro', classes: 'rounded red accent-2', 4000})
    return false
